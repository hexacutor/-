...
string Interface::com(string main)
{
    for (auto& c : main) c = toupper(c);
    if (main == "HELP")
    {
        ...
        if (err != 0)
        {
            return("Bad help file: " + manual);
        }
        else {
            ifstream help(manual);
            if (!help.is_open()) {
                return("Help file not found: " + manual);
            }
            else {
                string cur_line;
                log("Help file:");
        ...
        while (!ccc.eof())
        {
            ...
            else if (main == "GC")
            {
                if (cc->field_->inf != 2)
                {
                    ...
                    {
                        return("Bad values");
                        return("Default cloud created");
                        cc->buff.generate_cloud(0, 0, 1, 1, 100);
                        log("->oblaco sozdano s par-mi: " + to_string(1) + " " + to_string(1) + " " + to_string(0) + " " + to_string(0) + " " + to_string(100));
                        return("HELP-spisok komand");
                        ccc.clear();
                    }
                    else
                    {
                        cc->buff.generate_cloud(c_x, c_y, disp_x, disp_y, dots);
                        return("oblaco sozdano");
                        log("->oblaco sozdano s par-mi" + to_string(disp_x) + " " + to_string(disp_y) + " " + to_string(c_x) + " " + to_string(c_y) + " " + to_string(dots));
                    }
                }
                else
                {
                    return("oshibka:pole v drugom sostoyznii");
                    log("---> oshibka:pole v drugom sostoyznii");
                }
            }
            else if (main == "GEN")
            {
                ...
                {
                    return("Bad values");
                    return("Default random buffer created");
                    cc->buff.gen(500);
                    log("->buffer sgenerirovan,tochek: " + to_string(500));
                    ccc.clear();
                }
            ...
            {
                cc->field_->cmatrix();
                return("matrica sozdana");
                return("pole perevedeno v sosotoyanie 2(analis)");
            }
            else if (main == "KMC")
            {
                int k, p;
                ...
                {
                    return("Bad values");
                    return("KMeans with default values (3,3)");
                    cc->Kmc(3, 3);
                    log("KMeans with values (3,3) printed");
                    ccc.clear();
                }
                ...
                {
                    cc->Kmc(k, p);
                    return("proscanirovano(kmcore)");
                    return("pole raspechatano");
                    log("KMeans with values ( " + to_string(k) + "," + to_string(p) + ") printed");
                }
            }
            else if (main == "FOREL")
            {
                ...
                else
                {
                    ...
                    {
                        return("Bad values");
                        return("Forel with default value (0.1) and depth (2) printed");
                        cc->FOREL(0.1, 2);
                        log("Forel with value (0.1) and depth (2) printed");
                        ccc.clear();
                    }
                    cc->FOREL(r, dep);
                    return("Forel with value (" + to_string(r) + ") and depth (" + to_string(dep) + ") printed");
                    log("Forel with value (" + to_string(r) + ") and depth (" + to_string(dep) + ") printed");
                }
            }
            else if (main == "KM")
            {
                ...
                else
                {
                    ...
                    {
                        return("Bad values");
                        return("Kmeans with default value (5) printed");
                        cc->Km(5);
                        log("Kmeans with value (5) printed");
                        ccc.clear();
                    }
                    cc->Km(k);
                    return("Kmeans with value (" + to_string(k) + ") printed");
                    log("Kmeans with value (" + to_string(k) + ") printed");
                }
            }
            else if (main == "EM")
            {
                ...
                else
                {
                    ...
                    {
                        return("Bad values");
                        return("ExpMax with default value (3) printed (gif)");
                        cc->Km(3);
                        log("ExpMax with value (5) printed");
                        ccc.clear();
                    }
                    cc->Km(k);
                    return("ExpMax with value (" + to_string(k) + ") printed (gif)");
                    log("ExpMax with value (" + to_string(k) + ") printed");
                }
            }
            else if (main == "DBS")
            {
                if (cc->field_->inf == 2)
                {
                    ...
                    ccc >> del >> k;
                    cc->DBSCAN(del, k);
                    return("proscanirovano");
                    log("->proscanirovano(dbs)" + to_string(del) + to_string(k));
                    int err;
                    err = cc->fprintf_db(del, "core.txt", "Line.txt", "dust.txt");
                    if (err == -1)
                    {
                        return("file ne naiden");
                    }
                    else
                    {
                        return("pole raspechatano");
            ...
            {
                cc->IER();
                return("Hierarchy algorithm printed");
                log("Hierarchy algorithm printed");
            }
            else if (main == "BINARY")
            {
                ...
                else
                {
                    ...
                    {
                        return("Bad values");
                        return("Binary matrix with default value (0.1) created");
                        cc->binary(0.1);
                        log("Binary matrix with value (0.1) created");
                        ccc.clear();
                    }
                    else
                    {
                        if (cc->binary(k) < 0)
                        {
                            return("Binary error(no points)");
                            log("Binary error(no points)");
                        }
                        else
                        {
                            return("Binary matrix with (" + to_string(k) + "( created");
                            log("Binary matrix with (" + to_string(k) + "( created");
                        }
                    }
                }
            ...
            {
                {
                    ...
                    else
                    {
                        ...
                        {
                            return("Bad values");
                            return("Histogramm with default value (30) printed");
                            cc->hist(30);
                            log("Histogramm with value (30) printed");
                            ccc.clear();
                        }
                        cc->hist(k);
                        return("Histogramm with value (" + to_string(k) + ") printed");
                        log("Histogramm with value (" + to_string(k) + ") printed");
                    }
                }
            }
            ...
            else if (main == "DB")
            {
                ...
                return("unknown command in file");
                return("HELP-spisok komand");
            }
        }
    }
    ...
    {
        return("Field Length" + to_string(cc->field_->length()));
    }
    else if (main == "UNLOAD")
    {
        if (cc->field_->inf != 2)
    ...
    {
        cc->VORONOI();
        return("Voronoi diagram printed");
        log("Voronoi diagram printed");
    }
    else if (main == "BINARY")
    {
